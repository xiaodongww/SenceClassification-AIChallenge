...
state_dict = torch.load('./snapshots/densenet_truncated_dot5_step2_10.pytorch')
...
def train():
    ...
    for batch_idx, (data, target, otherfea) in enumerate(train_loader):
        ...
        turncated_loss_module = TruncatedLoss(0.5)
        ...
        loss_avg = loss_sum/float(batch_idx+1)
...
def test():
    ...
    state['test_loss'] = loss_sum / len(val_loader)
...
for epoch in range(maxepoch):
    ...
    train()
    test()
    torch.save(net.state_dict(), './snapshots/densenet_truncated_dot5_step3_{}.pytorch'.format(epoch))
